_Q='licensing-auth-token'
_P='machine'
_O='product'
_N='%s/license/request'
_M='localstack-pro'
_L='LOCALSTACK_AUTH_TOKEN'
_K='message'
_J='SSL_NO_VERIFY'
_I='name'
_H='credentials'
...
class AuthToken(Credentials):
	...
	def is_syntax_valid(A):
		if len(A.token)!=A.TOKEN_LENGTH:return _D
...
class LicenseV1ClientBase(LicensingClient):
	...
	def current_product_version_matches(G,license_):
		...
		try:
			...
			if A.products[0][_E].startswith('*'):return _C
			D=B[0],B[1];E=C[0],C[1]
...
class LicenseV1Client(LicenseV1ClientBase):
	...
	def _get_product_data(B):from localstack.constants import VERSION as A;return{_I:_M,_E:A}
	def activate_license_online(A,credentials,license):
		import requests as C;from localstack.utils.http import get_proxies as D;E=D();B=C.post(_N%constants.API_ENDPOINT,json.dumps({'license_id':license.id,_H:{'type':_Q,'token':credentials.token},_O:A._get_product_data(),_P:A._get_machine_data()}),verify=not config.is_env_true(_J),proxies=E)
	...
	def _request_license(A,credentials):
		import requests as C;from localstack.utils.http import get_proxies as D;E=D();B=C.post(_N%constants.API_ENDPOINT,json.dumps({_H:{'type':_Q,'token':credentials.token},_O:A._get_product_data(),_P:A._get_machine_data()}),verify=not config.is_env_true(_J),proxies=E)
	...
	def _server_error_to_exception(E,response):
		...
		try:
			C=B.json()
			if not C.get(_K):raise LicensingError(f"Unexpected license server error: {B.text}")
			A=C[_K]
...
class SelfSignedApiKeyLicensingClient(LicenseV1ClientBase):
	...
	def _request_self_signed_license(G,credentials):
		...
		try:
			A=H.post('%s/activate'%constants.API_ENDPOINT,json.dumps(K),verify=not config.is_env_true(_J),proxies=J);C=A.json()
			if A.status_code>=400:
				L=A.headers.get('Content-Type')
				if A.status_code==403:M=C[_K];raise CredentialsInvalidError(f"Activation key {B!r} is invalid or expired! Reason: {M}")
	...
	def _create_self_signed_license(B,credentials,version,encr_key_base64):C=credentials;A=datetime.now(tz=timezone.utc);license=LicenseV1(id=long_uid(),license_format='1',signature=_A,license_type='Pro',issue_date=A,expiry_date=A+B.default_expiry,products=[ProductInfo(name=_M,version=version)],license_status=LicenseStatus.ACTIVE,license_secret=_A,last_activated=A,reactivate_after=A+B.default_expiry);license.signature=LicenseSignerV1(C).calculate_signature(license);D=[b'LS0.1-SELFSIGN',base64.b64encode(license.signature.encode(_B)),encr_key_base64.encode(_B),base64.b64encode(C.to_bytes()),base64.b64encode(str(datetime.now(tz=timezone.utc)).encode(_B))];license.license_secret=base64.b64encode(b':'.join(D)).decode(_B);return license
	def decode_decryption_key(A,credentials,license):
		B,C,D=A._decode_license_secret(license.license_secret)
		if B!=license.signature:raise LicenseFormatError('Invalid license secret signature.')
		if credentials.to_bytes()!=C:raise LicenseFormatError('Could not decode license secret. If you recently rotated or changed your API key, please delete the cached license file in <localstack-volume>/cache/license.json')
		return D
	@staticmethod
	def _decode_license_secret(license_secret):
		try:
			C=base64.b64decode(license_secret).decode(_B);A=C.split(':',maxsplit=5)
			if len(A)!=5:raise LicenseFormatError()
			D=A[0]
			if D!='LS0.1-SELFSIGN':raise LicenseFormatError('Unknown license secret encoding scheme')
			E=base64.b64decode(A[1]).decode(_B);F=base64.b64decode(A[2]);G=base64.b64decode(A[3]);return E,G,F
		except ValueError as B:raise LicenseFormatError(f"key encoding error: {B}")from B
class LicensedLocalstackEnvironment:
	license:Optional[License]
	def __init__(A,client,parser=_A,serializer=_A):A.client=client;A.parser=parser or LicenseParser();A.serializer=serializer or LicenseSerializer();A.license=_A;A.license_file_path=_A;A._activated=_D;A._mutex=threading.RLock()
	@property
	def activated(self):return self._activated
	def activate(A,offline_only=_D):
		with A._mutex:
	...
	def activate_license(A,offline_only=_D):
		...
		if license:
			try:license=A.client.activate_license_online(C,license);A.client.validate_license(C,license);A._activated=_C;A.license=license;LOG.info('Successfully activated license %s 🔑✅',license.to_log_string());A.save_license();return
			except LicensingError as B:LOG.debug('There was an error activating the license: %s. Trying to get a new one.',B)
		license=A.client.request_new_license(C);A.client.validate_license(C,license);A._activated=_C;A.license=license;LOG.info('Successfully requested and activated new license %s 🔑✅',license.to_log_string());A.save_license()
	def _try_activate_offline(A,credentials):
		C=[]
		for B in A.get_license_file_read_locations():
			if not os.path.isfile(B):continue
			try:
				with open(B,'rb')as E:license=A.parser.parse(E.read());A.client.activate_license_offline(credentials,license);A._activated=_C;return license,B
	...
	def is_decryption_enabled(A):
		try:from localstack_ext.utils.decryption_check import decryption_check;return _C
		except ImportError:return _D
	def save_license(A):
		...
		try:
			os.makedirs(os.path.dirname(B),exist_ok=_C)
...
class DevLocalstackEnvironment(LicensedLocalstackEnvironment):
	def activate_license(A,offline_only=_D):LOG.debug('Using test license, skipping activation.');A._activated=_C
...
def get_licensed_environment():
	A=get_credentials_from_environment()
	if isinstance(A,ApiKeyCredentials):B=SelfSignedApiKeyLicensingClient()
...
def configure_container_licensing(cfg):
	...
	if os.path.exists(C):E=str(PurePosixPath(config.Directories.for_container().config)/LICENSE_FILE_NAME);B.volumes.add(D(C,E,read_only=_C))