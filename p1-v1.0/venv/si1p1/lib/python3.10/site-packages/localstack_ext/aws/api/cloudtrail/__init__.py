...
class CloudTrailARNInvalidException(ServiceException):
    ...
    This exception is also thrown when you call ``AddTags`` or
    ``RemoveTags`` on a trail, event data store, or channel with a resource
    ARN that is not valid.
...
class CloudtrailApi:
    ...
    def put_insight_selectors(
        ...
        error rate, the trail must log ``read`` or ``write`` management events.
        You can call ``GetEventSelectors`` on a trail to check whether the trail
