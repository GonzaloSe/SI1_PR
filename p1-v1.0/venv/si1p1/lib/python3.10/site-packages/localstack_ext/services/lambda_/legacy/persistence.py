...
from localstack.services.lambda_.lambda_api import LAMBDA_ZIP_FILE_NAME
from localstack.services.lambda_.lambda_models import lambda_stores_v1
from localstack.services.lambda_.lambda_utils import get_zip_bytes
...
def restore_backend_state():
	A=[]
	for(J,D)in lambda_stores_v1.items():
		for(K,B)in D.items():
			A.append(B)
